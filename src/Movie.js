        ...
        <MoviePopWapper ref={cover}>
          <MoviePop>
            ...
            <MoviePopDesc>
              ...
              <MoviePopDescDesc>
                {detailMovie.description_full.substr(0, 400)}
              </MoviePopDescDesc>
              ...
              <MovieDetailClose onClick={() => setOn(false)}>
                <BsX />
              </MovieDetailClose>
            </MoviePopDesc>
          </MoviePop>
...
const Movie = () => {
  ...
  const pageLimit = 20;
  const listNum = Array.from({
    length: parseInt(movieList.movie_count / limit),
  });
  ...
  const searchMovie = async () => {
    const r = await axios.get(
      `https://yts.mx/api/v2/list_movies.json?query_term=${inputList}`
    );
  ...
  return (
    <Wapper>
      ...
      <MovieSlideWrapper>
        ...
        <MovieSlideLeftArrow onClick={() => mainSlide.current.slickPrev()}>
          <BsArrowLeft />
        </MovieSlideLeftArrow>
        <MovieSlideRightArrow onClick={() => mainSlide.current.slickNext()}>
          <BsArrowRight />
        </MovieSlideRightArrow>
      </MovieSlideWrapper>
      ...
      <Routes>
        ...
        <Route
          path="/detail/:id"
          element={<DetailMovie movie={movie} on={on} setOn={setOn} />}
        />
        <Route
          path="/search/:id"
          element={<SearchMovie search={search} on={on} setOn={setOn} />}
        />
      </Routes>
      ...
      <ListBtnWrapper>
        ...
        {listNum
          .map((_, idx) => {
            return (
              <ListBtn onClick={() => setPageNum(idx + 1)} key={idx}>
                {idx + 1}
              </ListBtn>
            );
          })
